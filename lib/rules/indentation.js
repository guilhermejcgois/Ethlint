...
module.exports = {
    ...
    create: function(context) {
        ...
        function inspectInlineAssemblyBlock(emitted) {
          let node = emitted.node;
          ...
          if (sourceCode.getLine(node) === sourceCode.getEndingLine(node)) {
            return;
          }
          ...
          function inspectAssemblyItem(blockIndent, blockIndentDesc, blockItem) {
            let prevChars = sourceCode.getPrevChars(blockItem, blockIndent.length+1);
          ...
          let indentSum = 1;
          if (node.type === 'InlineAssemblyBlock' && sourceCode.getTextOnLine(sourceCode.getLine(node)).trim().length === 1) {
            indentSum = 2;
          }

          //indentation of items inside block should be 1 level greater than that of parent
          const blockIndent = getIndentString(BASE_INDENTATION_STYLE, currentIndentLevel + indentSum);
          const blockIndentDesc = getIndentDescription(BASE_INDENTATION_STYLE, currentIndentLevel + indentSum);

          node.items.forEach(inspectAssemblyItem.bind(null, blockIndent, blockIndentDesc));
        }
        ...
        let response = {
            ...
            InlineAssemblyBlock: inspectInlineAssemblyBlock
        };
